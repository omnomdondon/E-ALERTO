...
app.post('/api/reports', upload.single('image_file'), async (req, res) => {
    ...
    try {
        ...
        const { classification, measurement, location, timestamp, description } = req.body;
        ...
        res.status(500).json({ success: false, message: 'Failed to save report', shouldRedirect: false });
    }
});
...
app.post('/api/upload-detected-image', upload.single('image_file'), validateImage, async (req, res) => {
    try {
        if (!req.file) {
...
//         // ✅ Dummy detection response
//         const detections = [
//             { label: 'pothole', confidence: 0.92 },
//             { label: 'crack', confidence: 0.81 }
//         ];

//         const timestampNow = Date.now();
//         const fileName = `detect_${timestampNow}.jpeg`;
...
//         res.status(200).json({
//             detections,
//             image: uploadStream.id.toString(), // 🔥 GridFS ObjectId
//         });
//     } catch (err) {
//         console.error('❌ Image detection error:', err);
//         return res.status(500).json({ message: 'Failed to process image' });
